...
def main() -> None:
    ...
    parser.add_argument("--record", default=None, help="JSONL output path (default: runs/demo-SEED.jsonl)")
    ...
    record_path = Path(args.record or f"runs/demo-{args.seed}.jsonl")
    ...
    for _ in range(steps):
        ...
        if abs(engine.time - 20.0) < 0.05:
            engine.fail_control()
        if abs(engine.time - 24.0) < 0.05:
            failed_after_control = "drone-2"
            engine.fail_drone(failed_after_control, source="killer-demo")
        if abs(engine.time - 32.0) < 0.05:
            engine.inject_event(
                ScenarioEvent(timestamp=engine.time, type="NETWORK_PARTITION", affected_nodes=["drone-1", "drone-3"], severity=1, duration=6)
            )
        if abs(engine.time - 45.0) < 0.05:
            gps_node = "drone-3" if engine.world.is_online("drone-3") else min(engine._online_node_ids())
            engine.inject_event(
                ScenarioEvent(timestamp=engine.time, type="GPS_OUTAGE", affected_nodes=[gps_node], severity=1, duration=10)
            )
        ...
        if round(engine.time * 10) % 10 == 0:
            ...
            print(
                f"t={engine.time:05.1f} capability={snapshot.mission_capability:4.0%} effectiveness={snapshot.mission_effectiveness:4.0%} "
        ...
        if args.realtime:
            time.sleep(0.1)
    alpha = next(region for region in snapshot.world_knowledge.regions if region.region_id == "ALPHA")
    sync_state = "SYNCHRONIZED" if snapshot.world_knowledge.world_model_sync_lag <= engine.config.status_interval else "DIVERGED"
    print(
        f"final mission capability={snapshot.mission_capability:.0%} effectiveness={snapshot.mission_effectiveness:.0%} "
        f"coverage={alpha.coverage:.0%} fresh={alpha.fresh_coverage:.0%} "
        ...
        f"world_model={sync_state} replay={record_path}"
    )
    recorder.close()
