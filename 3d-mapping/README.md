...
The local screen uses one translucent command dock centered at the bottom: **Deploy**, the drone selector, and **Fly** are always available. Secondary individual, batch, mission, playback, and reset tools stay in the single **More** drawer. Placement controls open contextually inside that drawer. Switching tabs preserves selection and flight state; picking up a batch opens its tab automatically.
...
The scene starts with an empty fleet and the camera circling the Washington Monument. Click **Deploy**, navigate to a surface, choose height above it and confirm **Deploy here**. Placement records no route. Choose a drone in the central selector and click **Fly** to pilot it. Colors cycle through eight choices; Reset all drones removes the fleet and its routes. Release markers are prisms and the live arrow is three meters long.
