...
The local screen uses only one short translucent text command line centered at the bottom. Type `/` to see direct commands such as `/deploy`, `/fly`, `/speed`, `/return`, and `/reset`; common phrases such as “deploy 3 survey drones” and “move forward 50” execute instantly. Other plain-language instructions go through the backend's validated local-LLM mission compiler and are converted into local visual missions. Command feedback becomes the empty input's placeholder instead of occupying another panel. WASD pilots relative to the screen, Q/E turns, R/F or Space/Shift changes altitude, and Escape releases.

Rendering work is kept out of the hot path where possible: the removed detail/telemetry panels perform no per-frame DOM work, quadcopter parts share one cached pose per render frame, terrain and photogrammetry use performance-oriented screen-space error settings, and Cesium uses FXAA instead of multisample antialiasing. Drone physics and camera movement continue at the display frame rate.
