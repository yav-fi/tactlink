...
The local screen uses one translucent command dock centered at the bottom: **Deploy**, the drone selector, and **Fly** are always available. While piloting, compact ↶/↷ controls appear for rotation; Q/E provide the same controls from the keyboard. Secondary individual, batch, mission, playback, and reset tools stay in the single **More** drawer. Placement controls open contextually inside that drawer. Switching tabs preserves selection and flight state; picking up a batch opens its tab automatically.
...
The scene starts with an empty fleet and the camera circling the Washington Monument. Click **Deploy**, then click one visible map surface: a normal drone is placed there immediately at the safe 20 m default height. There is no confirmation button for single deployment. Choose a drone in the central selector and click **Fly** to pilot it. Colors cycle through eight choices; Reset all drones removes the fleet and its routes. Release markers are prisms.
