...
The local screen uses one short translucent text command line centered just above a 22 px, full-width telemetry strip. Type `/` to see direct commands such as `/deploy`, `/fly`, `/speed`, `/return`, and `/reset`; common phrases such as “deploy 3 survey drones” and “move forward 50” execute instantly. Other plain-language instructions go through the backend's validated local-LLM mission compiler and are converted into local visual missions. The footer reports the selected drone, state, latitude, longitude, altitude, rendering FPS, and the latest command result. WASD pilots relative to the screen, Q/E turns, R/F or Space/Shift changes altitude, and Escape releases.

Rendering work is kept out of the hot path where possible: text and telemetry DOM updates run at 10 Hz, quadcopter parts share one cached pose per render frame, terrain and photogrammetry use performance-oriented screen-space error settings, and Cesium uses FXAA instead of multisample antialiasing. Drone physics and camera movement continue at the display frame rate.
