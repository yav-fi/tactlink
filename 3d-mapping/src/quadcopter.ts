...
export function addQuadcopterParts(viewer: Cesium.Viewer, id: string, position: () => Cesium.Cartesian3, orientation: () => Cesium.Quaternion, color: () => Cesium.Color, animationTime = () => performance.now() / 1000): Cesium.Entity[] {
  ...
  for (const x of [-1, 1]) for (const y of [-1, 1]) {
    ...
    for (let blade = 0; blade < 2; blade++) {
      part(`prop_${name}_${blade}`, new Cesium.Cartesian3(x * 0.28, y * 0.28, 0.08), new Cesium.Cartesian3(0.32, 0.025, 0.012), () => blade * Math.PI / 2 + Math.PI / 4 + (animationTime() * 18 * x * y) % (Math.PI * 2));
    }
  }
  return parts;
}
